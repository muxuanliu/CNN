...
from model import AlexNet  # model.py中定义了LeNet模型
from tqdm import tqdm  # 导入tqdm库，用于显示进度条

# 定义数据加载和处理函数
def train_val_data_process():
    # 加载FashionMNIST数据集，Resize到227尺寸，并转换为Tensor
...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    ...
    # 训练结束，保存最佳模型权重
    torch.save(best_model_wts, './weight/best_model.pth')

    # 如果当前epoch为总epoch数，则保存最终模型权重
    if epoch == num_epochs:
        torch.save(model.state_dict(), f'./weight/{num_epochs}_model.pth')
...
if __name__ == "__main__":
    ...
    train_process = train_model_process(model, train_dataloader, val_dataloader, num_epochs=10)
